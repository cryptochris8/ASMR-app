...
export class PlayerHUD extends UIPanel {
  ...
  protected render(): void {
    ...
    this.element.innerHTML = `
      <nav class="player-top-bar" aria-label="Scene navigation">
        <button type="button" class="player-btn player-back-btn" aria-label="Back to home">
        ...
        <div class="player-scene-label" aria-live="polite">${scene?.name ?? ''}</div>
        <div class="player-timer-badge" role="timer" aria-live="off" aria-label="Sleep timer remaining" style="display: ${timerActive ? 'block' : 'none'}"></div>
      </nav>

      <div class="player-persistent-timer" role="timer" aria-live="off" aria-label="Sleep timer remaining" style="display: ${timerActive ? 'block' : 'none'}"></div>

      <div class="player-bottom-bar" role="toolbar" aria-label="Playback controls">
        <button type="button" class="player-btn" data-action="timer" aria-label="${timerActive ? 'Sleep timer running, tap to manage' : 'Set sleep timer'}" aria-pressed="${timerActive}">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="${timerActive ? CONFIG.colors.timerActive : CONFIG.colors.textMuted}" stroke-width="1.5" aria-hidden="true" focusable="false">
        ...
        <button type="button" class="player-btn" data-action="mixer" aria-label="Open sound mixer">
        ...
        <button type="button" class="player-btn" data-action="mute" aria-label="${muted ? 'Unmute audio' : 'Mute audio'}" aria-pressed="${muted}">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="${muted ? CONFIG.colors.danger : CONFIG.colors.textMuted}" stroke-width="1.5" aria-hidden="true" focusable="false">
            ${muted
        ...
        <button type="button" class="player-btn" data-action="dim" aria-label="Dim screen for sleep">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="${CONFIG.colors.textMuted}" stroke-width="1.5" aria-hidden="true" focusable="false">
  ...
  private applyStyles(): void {
    ...
    style.textContent = `
      ...
      .player-btn:hover { background: ${CONFIG.colors.surfaceLight}30; }
      .player-btn:active { background: ${CONFIG.colors.surfaceLight}40; transform: scale(0.96); }
      .player-btn:focus-visible {
        outline-color: ${CONFIG.colors.primary};
        background: ${CONFIG.colors.surfaceLight}30;
      }
      .player-btn-label {
